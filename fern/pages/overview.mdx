...
<Info title="Streaming Speech-to-Text">
  ...
  reference](/docs/api-reference/streaming-api/universal-streaming).
</Info>
...
The AssemblyAI API enforces the following rate limits to ensure availability for all users:

- **Global API rate limit**: All accounts are limited to **20,000 requests per five minutes** across all endpoints. If you exceed this limit, the API responds with a `429` status code. Use exponential backoff with jitter when retrying. For high-volume async transcription workloads, see [Scaling async transcription](/docs/deployment/account-management#scaling-async-transcription).
- **LLM Gateway rate limit**: LLM Gateway endpoints have a separate per-model rate limit within a 60-second time window.
