...
<Note>
  ...
  `api.eu.assemblyai.com`. The EU endpoint is available for **Async STT**.
  **Streaming STT** is not currently supported.
</Note>
...
To ensure the LLM Gateway API remains available for all users, you can only make a limited number of requests within a 60-second time window. Only LLM Gateway requests are rate limited.
