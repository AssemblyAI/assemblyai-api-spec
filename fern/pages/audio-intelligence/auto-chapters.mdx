...
<Warning title="Model compatibility">
  Auto Chapters is only compatible with Universal-2. Using `auto_chapters=true`
  with Universal-3-Pro will result in a server error. To use Auto Chapters,
  either set `speech_models` to `["universal-2"]`, or set `speech_models` to
  `["universal-3-pro", "universal-2"]` with `auto_chapters` set to `false` and
  use a separate Universal-2 request for chapters.
</Warning>
