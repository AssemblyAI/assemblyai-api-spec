...
In this guide, we'll show you how to use the [Summarization model](../03-audio-intelligence/summarization.mdx) to automatically generate summaries of your virtual meetings, so you can quickly review important information and take action based on the insights gathered from the conversations.
...
You can find all possible [summary models and types here](../03-audio-intelligence/summarization.mdx#types-and-models). Note that if the `summary_model` is specified, `summary_type` is also a required parameter, and vice versa.
