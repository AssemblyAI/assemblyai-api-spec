...
<Tabs groupId="language">
    ...
    GLADIA_API_KEY = "fee8d14b-57b6-4288-8e6d-52f050bc930a"
    ```
    </Tab>
    <Tab language="aai" title="AssemblyAI">
...
- **Partial Transcripts** - AssemblyAI streams Partial updates (interim results) automatically. There’s no switch to toggle. Expect fast, token-level updates that refine until each Final is emitted.
...
- **Formatted vs. Raw Finals** – `"Final"` objects include a boolean `formatted_finals`. When you set `formatted_finals` to `True`, punctuation/casing appears in the Final Transctript, so you can toggle display styles on the fly.
