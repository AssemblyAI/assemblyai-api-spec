...
To specify a speech model, set the `SpeechModels` field in `TranscriptOptionalParams`:

```go
params := &aai.TranscriptOptionalParams{
	SpeechModels: []aai.SpeechModel{aai.SpeechModelSlam1},
}
```
...
```go
params := &aai.TranscriptOptionalParams{
	SpeechModels: []aai.SpeechModel{aai.SpeechModelSlam1, aai.SpeechModelUniversal},
}
```
...
```java
var params = TranscriptOptionalParams.builder()
        .speechModels(List.of(SpeechModel.SLAM_1))
        .build();
```
...
```java
var params = TranscriptOptionalParams.builder()
        .speechModels(List.of(SpeechModel.SLAM_1, SpeechModel.UNIVERSAL))
        .build();
```
...
To specify a speech model, set the `SpeechModels` property in `TranscriptParams`:

```csharp
var transcript = await client.Transcripts.TranscribeAsync(new TranscriptParams
{
    AudioUrl = audioUrl,
    SpeechModels = [SpeechModel.Slam1]
});
```
...
{
    AudioUrl = audioUrl,
    SpeechModels = [SpeechModel.Slam1, SpeechModel.Universal]
});
```
...
To specify a speech model, set the `speech_models` parameter in `transcribe()`:

```ruby
transcript = client.transcripts.transcribe(
  audio_url: audio_url,
  speech_models: [AssemblyAI::Transcripts::SpeechModel::SLAM_1]
)
```
...
transcript = client.transcripts.transcribe(
  audio_url: audio_url,
  speech_models: [
    AssemblyAI::Transcripts::SpeechModel::SLAM_1,
    AssemblyAI::Transcripts::SpeechModel::UNIVERSAL
  ]
)
```
...
## Step 4: Submit audio for transcription
...
## Step 5: Enable additional AI models
...
- For more ways to analyze your audio data, explore our [Speech Understanding](https://www.assemblyai.com/products/speech-understanding).
