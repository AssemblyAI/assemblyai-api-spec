...
<Tabs groupId="language">
  ...
  <Tab language="typescript" title="TypeScript">
      <Steps>
        ...
        Create a `transcribe` function and specify a URL to the audio you want to transcribe. Add a `data` object and pass the `audioFile` throught the `audio_url` parameter. The URL needs to be accessible from AssemblyAI's servers. For a list of supported formats, see [FAQ](https://support.assemblyai.com/).
