...
<Tabs>
  <Tab title="Medical consultation" language="python">
  ```
    "keyterms_prompt": [
  ...
  <Tab title="Professional therapy session" language="python">
  ```
    "keyterms_prompt": [
  ...
  <Tab title="Veterinary exam language="python">
  ```
    "keyterms_prompt": [
...
Prompting with the `keyterms_prompt` parameter is a good option when you have a specific terminology or vocabulary that you would like to fine-tune the model with. `keyterms_prompt` works best with specific use cases where specific words and phrases can be identified.
