...
Slam-1 represents a fundamental shift in speech recognition technology. By combining LLM architecture with our best-in-class ASR encoders, we've created the world's first Audio Language Model (ALM) optimized explicitly for speech-to-text tasks.
...
To improve the transcription accuracy, you can boost certain words or phrases that appear frequently in your audio file. You can boost up to 100 unique keywords and phrases, with each phrase containing up to 6 words.

To boost words or phrases, include the `word_boost` parameter in your request as shown in the code example below. Words and phrases should be formatted in the way that you would like to see them returned in your transcripts.
