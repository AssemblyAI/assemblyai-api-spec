...
Improve transcription accuracy by leveraging Slam-1's contextual understanding capabilities by prompting the model with a description of your audio in plain English. This allows the model to understand the broader context of your audio file and make more intelligent transcription decisions.

Provide up to 1,500 words of context about your audio:
...
<Warning>
Context prompting via the `prompt` parameter is currently experimental and in an early form of release, so your mileage may vary when using this feature.
...
Use the `prompt`parameter when you have general context about the audio content. Context prompting works best when you would rather pass a high-level description than specific words or phrases.
