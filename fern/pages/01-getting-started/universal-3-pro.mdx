...
Universal 3 Pro represents a fundamental shift in speech recognition technology. With a multimodal LLM architecture that jointly optimizes an LLM and our best-in-class speech encoder, we've created the world's first Speech Language Model explicitly optimized for speech-to-text tasks—enabling you to adjust transcription nuances directly through prompting.
...
- **Universal 3 Pro Multilingual**: Supports English, Spanish, German, French, Portuguese, and Italian.

<Note>

If your audio contains only English speech, use Universal 3 Pro English; for code-switching scenarios, Universal 3 Pro Multilingual is recommended.
...
See [Select the speech model](/docs/speech-to-text/pre-recorded-audio/select-the-speech-model) for more information on how to specify speech models, including auto routing behavior when multiple models are specified.
...
One approach is to prompt the model with key terminology in the form of a list of words and phrases. The other uses natural-language prompts to convey the nuances you want to capture or the contextual information relevant to your audio. Continue reading for more information on these two approaches.

### Key-term prompting of words and phrases
...
Provide up to 1,000 domain-specific words or phrases (maximum 6 words per phrase) that may appear in your audio using the optional `keyterms_prompt` parameter:
...
While we support up to 1,000 keywords and phrases, actual capacity may be lower due to internal tokenization and implementation constraints.
...
```txt wordWrap
["Jane Doe", "cognitive behavioral therapy", "major depressive disorder", "generalized anxiety disorder", "ADHD", "trauma-informed care", "Lexapro 10mg", "psychosocial assessment", "therapeutic alliance", "emotional dysregulation", "GAD-7", "PHQ-9", "Citalopram", "Lorazepam"]
```
...
### Should I use key-term prompting or natural-language prompting?

Key-term prompting with the `keyterms_prompt` parameter is a good option when you have a specific terminology or vocabulary that you would like to fine-tune the model with. Key-term prompting works best with targeted use cases where specific words and phrases can be identified.

Natural-language prompting with the `prompt` parameter is a good option when you know the context of the audio, perhaps derived from some metadata, but exact terminology may be too broad or numerous to capture in a list. Natural-language prompting works best when you would rather pass a high-level description than specific words or phrases.
