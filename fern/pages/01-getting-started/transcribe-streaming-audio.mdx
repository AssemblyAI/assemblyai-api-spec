...
To get a better understand of the turn event and the message sequences, check out our [Message Sequence Breakdown](/docs/speech-to-text/universal-streaming/universal-streaming/message-sequence) page. This object is how you'll receive your transcripts.
</Tip>
...
<Steps>
  <Step>
  In the `main` function create a `client`. and and connect to the streaming service:
...
<Steps>
<Step>
In the `run` function, initialize the audio stream.
...
<Step>
Next, create a WebSocket connection to the real-time service:
```python
    # Create WebSocketApp
    ws_app = websocket.WebSocketApp(
        API_ENDPOINT,
...
<Note title="Initialize WebSocket">
The WebSocket was initialized in at the beginning of the `main` function in Step 4.
</Note>


</Step>
<Step>
Create the function that initializes the audio stream. This function is called within the `main` function from Step 4.
```ts
function startMicrophone() {
  try {
    micInstance = mic({
