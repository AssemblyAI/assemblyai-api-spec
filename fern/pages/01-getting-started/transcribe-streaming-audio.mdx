...
Create a new streaming service from the AssemblyAI client and add it to your async function. If you don't set a sample rate, it defaults to 16 kHz. Configure your API key in th streaming service as well.
...
* `on_turn` – Handles each transcription turn and optionally enables formatted finals.
...
<Steps>
  ...
  <Step>
  Next, create a microphone stream and begin transcribing audio. Make sure the `sample_rate` matches the value you specified in the connection parameters when initializing the streaming client.
