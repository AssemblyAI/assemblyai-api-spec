...
The advanced usage examples below show the `understanding_body` payload sent to the Speech Understanding API. For setup, transcription, and polling code, see the full examples above.

### Identifying speakers by role

To identify speakers by role instead of name, use the `speaker_type` parameter with a value of "role":
...
For more accurate identification, use the `speakers` parameter instead of `known_values` to provide descriptions and metadata:
