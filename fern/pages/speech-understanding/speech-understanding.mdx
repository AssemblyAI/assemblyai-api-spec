...
Speaker Identification supports two approaches for specifying speaker information:

- **`known_values`** - Pass a simple list of names or roles. Best when you just need basic labels.
- **`speakers`** - Pass names or roles along with descriptions and custom metadata. Best when you need more accurate identification, such as when speakers have similar voices but distinct roles or topics.

You can identify speakers by name or by role:

- **Know the speakers' names?** Use `speaker_type: "name"` with the names in `known_values` or `speakers`.
- **Know their roles but not names?** Use `speaker_type: "role"` with roles like `"Interviewer"` or `"Agent"` in `known_values` or `speakers`.
- **Need better accuracy?** Use `speakers` with `description` fields that provide context about what each speaker typically discusses.

## How to use Speaker Identification

Include the `speech_understanding` parameter in your transcription request to transcribe and identify speakers in a single step.
...
<Note>
For the API reference for adding Speaker Identification to an existing transcript, see [Using Speaker Identification on an existing transcript](/docs/speech-understanding/speaker-identification-existing-transcript).
</Note>
