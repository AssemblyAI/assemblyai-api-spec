...
For more accurate speaker identification, you can use the `speakers` parameter instead of `known_values`. The `speakers` parameter lets you provide additional metadata about each speaker, including a `description` that helps the model identify speakers based on conversational context.
...
#### Example with roles and descriptions
    ...
    "request": {
      "speaker_identification": {
        ...
        "speakers": [
          {
            "role": "captain",
            "description": "Flies ship, instructs crew, manages distress calls"
          },
          {
            "role": "crew",
            "description": "Assists with completion of the mission"
          },
          {
            "role": "advertiser",
            "description": "Talks about products near the end of the episode"
          }
        ]
      }
    }
...
<Note>
  ...
  when you want to provide descriptions alongside names or roles to improve
  identification accuracy. Use `known_values` when you only need to provide a
  simple list of names or roles without additional context.
</Note>
...
| `speaker_identification.speakers`                     | array  | Conditional | An array of speaker objects with metadata. Use as an alternative to `known_values` when you want to provide additional context about each speaker. Use `speakers` or `known_values`, not both.                                                     |
