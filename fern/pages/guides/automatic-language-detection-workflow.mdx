...
hide-nav-links: true
description: "Automatically detect the language using a cost-effective Universal-2 ALD workflow"
---

In this guide, you'll learn how to perform automatic language detection (ALD) separately from the transcription process. For the transcription, the file then gets routed to either [Universal-3-Pro or Universal-2](/docs/speech-to-text/pre-recorded-audio/select-the-speech-model), depending on the supported language.

This workflow is designed to be cost-effective, slicing the first 60 seconds of audio and running it through Universal-2 with ALD, which detects 99 languages, at a cost of $0.002 per transcript for this language detection workflow (not including the total transcription cost).

Performing ALD with this workflow has a few benefits:

- Cost-effective language detection
- Ability to detect 99 languages
- Ability to use Universal-2 as fallback if the language is not supported in Universal-3 Pro
- Ability to enable [Speech Understanding models](/audio-intelligence) if the [language is supported](/docs/pre-recorded-audio/supported-languages)
...
Create a set with all supported languages for Universal. You can find them in our [documentation here](/docs/pre-recorded-audio/supported-languages).
...
- `detect_language()` performs language detection on the [first 60 seconds](https://www.assemblyai.com/docs/api-reference/transcripts/submit#request.body.audio_end_at) of the audio using Universal-2 and returns the language code.
- `transcribe_file()` performs the transcription using Universal-3-Pro or Universal-2 depending on the identified language.
