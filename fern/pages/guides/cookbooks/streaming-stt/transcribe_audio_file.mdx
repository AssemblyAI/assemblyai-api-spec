...
turn_params = ["end_of_turn_confidence_threshold", "min_turn_silence", "max_turn_silence"]
if not any(getattr(streaming_params, p, None) is not None for p in turn_params):
    ...
    print("https://www.assemblyai.com/docs/streaming/turn-detection#quick-start-configurations\n")
...
Before we begin, make sure you have an AssemblyAI account and an API key. You can [sign up](https://assemblyai.com/dashboard/signup) and get your API key from your [dashboard](https://www.assemblyai.com/app/api-keys).
...
A warning is printed if default turn detection parameters are used. This is fine for testing, but for best accuracy and optimal performance, see our [recommended settings](/docs/streaming/turn-detection#quick-start-configurations).
...
https://www.assemblyai.com/docs/streaming/turn-detection#quick-start-configurations
