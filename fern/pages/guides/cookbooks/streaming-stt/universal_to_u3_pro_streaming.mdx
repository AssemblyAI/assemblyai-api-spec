...
**Universal-3 Pro** natively code-switches between 6 languages in a single model — no separate multilingual model needed: English, Spanish, German, French, Portuguese, Italian. To guide toward a specific language, prepend `Transcribe <language>.` to the default prompt. ([Source](/docs/streaming/universal-3-pro#prompting-language))

**Language Detection:** Universal Streaming supports the `language_detection` connection parameter (`true`/`false`, default `false`) with the multilingual model. When enabled, Turn messages include `language_code` and `language_confidence` fields.
