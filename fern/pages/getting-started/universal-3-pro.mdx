...
When no `prompt` is provided, Universal-3 Pro automatically applies a default prompt based on the audio length and `keyterms_prompt` parameter:

- If the audio is **longer than 10 seconds** and `keyterms_prompt` is null or an empty list, the following default prompt is used:
