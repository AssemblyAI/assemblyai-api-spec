...
// --- Main function ---
async function run() {
  console.log("Starting AssemblyAI streaming transcription with LLM Gateway...");
...
## Step 2: Configure LLM Gateway

In this step, you'll configure the `llm_gateway` parameter that will be passed to the streaming connection. This parameter tells the streaming API to process your transcripts with LLM Gateway in real-time.
...
The `llm_gateway` parameterisa JSON-stringified object that follows the same interface as the [LLM Gateway chat completions API](/docs/llm-gateway/chat-completions). It accepts the following fields:
...
In this step, you'll stream audio from your microphone and receive LLM Gateway analysis in real-time after each completed turn.

<Steps>

<Step>

Set up event handlers to receive transcripts and LLM Gateway responses.
...
When you configure the `llm_gateway` parameter in the connection, LLM Gateway automatically processes each completed turn and sends back analysis responses through the streaming connection. The responses are received through the event handlers you set up in the previous step:

- **Python SDK**: Responses arrive via the `LLMGatewayResponseEvent` handler
- **Python WebSocket**: Responses arrive as `LLMGatewayResponse` messages in `on_message`
- **JavaScript SDK**: Responses arrive via the `llmGatewayResponse` event handler
- **JavaScript WebSocket**: Responses arrive as `LLMGatewayResponse` messages in the message handler

No separate API call is needed - the analysis happens automatically for each turn.

</Step>
<Step>

Run the streaming session. LLM Gateway will analyze transcripts in real-time as turns complete.
...
LLM Response:
The speaker introduces themselves as Sonny.

Transcript:
I am a voice agent.

LLM Response:
The speaker identifies as a voice agent.
...
- [Select the speech model](/docs/streaming/select-the-speech-model)
