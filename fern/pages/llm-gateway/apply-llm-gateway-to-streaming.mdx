...
def on_turn(self: Type[StreamingClient], event: TurnEvent):
    ...
    if event.end_of_turn:
        conversation_data += f"{event.transcript}\n"
...
<Tab language="python-sdk" title="Python SDK" default>
Install the AssemblyAI Python SDK and requests via pip:

```bash
pip install "assemblyai[extras]" requests
```
...
The `llm_gateway` parameteris a JSON-stringified object that follows the same interface as the [LLM Gateway chat completions API](/docs/llm-gateway/chat-completions). It accepts the following fields:
