...
- Turn detection with configurable silence thresholds for natural speech boundaries
- Mid-session configuration updates via `UpdateConfiguration` messages
- `forceEndpoint` to manually trigger turn boundaries
- VAD threshold tuning for noisy environments
- Inactivity timeout for auto-terminating idle sessions

**Streaming Models**:

- **Universal-Streaming** — Default streaming model with low latency (~300ms), supports English and multilingual (English, Spanish, French, German, Portuguese, Italian)
- **Universal-3 Pro Streaming** (`u3-rt-pro`) — Highest accuracy streaming model, supports prompting and `speechStarted` events. Best for meeting notetakers that demand the highest quality
- **Whisper-Streaming** (`whisper-rt`) — 99+ language support for broad multilingual coverage
...
async def transcribe_meeting_async(audio_source: str) -> Dict:
    ...
    config = aai.TranscriptionConfig(
        ...
        # Boost accuracy of meeting-specific vocabulary (up to 1000 terms for Universal-3-Pro)
...
CONNECTION_PARAMS = {
    ...
    # Default min_turn_silence is 400ms, default max_turn_silence is 1280ms
...
<Note>
  ...
  [tweak these settings](https://www.assemblyai.com/docs/streaming/universal-streaming/turn-detection) to get
...
You can add speaker identification to an existing transcript by re-submitting with the `speech_understanding` parameter. This is useful when you get speaker names after the transcription completes, or when building iterative workflows where users confirm speaker identities.
...
config = aai.TranscriptionConfig(
    ...
    # Enable translation via Speech Understanding
    speech_understanding={
        "request": {
            "translation": {
...
# Access translated text from speech_understanding response
print("\nTranslation (English):")
print(transcript.json_response["speech_understanding"]["results"][0]["translated_texts"]["en"])
```
...
config = aai.TranscriptionConfig(
    ...
    speech_understanding={
        "request": {
            "translation": {
                "target_languages": ["en"],
                "match_original_utterance": True  # Get per-utterance translations
            }
        }
    }
)
...
async def process_international_meeting(audio_url: str, target_language: str = "en"):
    ...
    config = aai.TranscriptionConfig(
        ...
        # Translate via Speech Understanding
        speech_understanding={
            "request": {
                "translation": {
    ...
    su_results = transcript.json_response.get("speech_understanding", {}).get("results", [{}])[0]

    return {
        "original_transcript": transcript.text,
        "translated_texts": su_results.get("translated_texts", {}),
        "summary": transcript.summary,
...
CONNECTION_PARAMS = {
    "sample_rate": 16000,
    "format_turns": True,
    "keyterms_prompt": keyterms,  # Pass as list; use urlencode(params, doseq=True)
}
```
