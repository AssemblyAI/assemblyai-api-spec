...
For more details, see the [Universal-3 Pro Streaming documentation](https://www.assemblyai.com/docs/streaming/universal-3-pro-streaming).
...
# Channel 0 = Agent, Channel 1 = Customer (typical layout)
for utterance in transcript.utterances:
    role = "Agent" if utterance.channel == "0" else "Customer"
...
### Multichannel with Call Transfers

When calls are transferred, you may have multiple agents on one channel. Use `speaker_labels` alongside `multichannel` for this scenario:

```python
config = aai.TranscriptionConfig(
    speech_models=["universal-3-pro", "universal-2"],
    multichannel=True,
    speaker_labels=True,  # Also diarize within each channel for transfers
    speaker_options=SpeakerOptions(
        min_speakers_expected=2,
        max_speakers_expected=5  # Account for transfers
    ),
)
```
...
CONNECTION_PARAMS = {
    ...
    # Contact center turn detection - wait for natural pauses
    "min_turn_silence": 400,  # Default - good for contact center calls
    "max_turn_silence": 1500,  # Slightly longer for customers explaining issues
...
def assemblyai_webhook():
    ...
    if status == "completed":
        process_completed_call(data)
    elif status == "error":
        log_transcription_error(transcript_id, data["error"])

    return jsonify({"received": True}), 200

def process_completed_call(transcript_data):
    ...
    # Push summary to CRM
    push_to_crm(transcript_data["id"], summary)

    # Run QA scoring
    qa_score = score_call_quality(utterances)
    save_qa_score(transcript_data["id"], qa_score)
```
...
- [Universal-3 Pro Streaming Documentation](https://www.assemblyai.com/docs/streaming/universal-3-pro-streaming)
