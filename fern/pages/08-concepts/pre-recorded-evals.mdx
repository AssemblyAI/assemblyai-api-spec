...
Before calculating any metrics, both reference (ground truth) and hypothesis (model generated) texts need to be normalized to ensure a fair comparison.
...
Vibe-evals are useful to determine the qualitative difference between STT providers that metrics might not capture. For example, how certain keyterms can are transcribed make or break a transcript, even if the rest of the transcript is more accurate for other terms.
...
## Current AssemblyAI Pre-recorded Speech-to-text Benchmarks
...
<Note>
    ...
    If you wish to check out third-party benchmarks, we'd highly recommend the [Hugging Face ASR Leaderboard](https://huggingface.co/spaces/hf-audio/open_asr_leaderboard).
</Note>
...
| audio | 8.56% | 16.50% | - |
| broadcast | 4.40% | 24.55% | - |
| sitelock | 10.71% | - | - |
| noisy | 10.43% | - | - |
| webinar | 5.87% | - | - |
| commonvoice | 6.48% | - | - |
| earnings21 | 9.37% | - | - |
| librispeech_test_clean | 1.60% | - | - |
| librispeech_test_other | 2.86% | - | - |
| meanwhile | 4.42% | - | - |
| tedlium | 7.31% | - | - |
| rev16 | 10.31% | - | - |
