...
| `translation.match_original_utterance`       | boolean | No        | Whether to include translated texts for utterances. Defaults to `false`. When `true`, uses formal pronouns and grammatical forms. Requires `speaker_labels` to be set to "true" in the request.    |
