...
To select speech models, use the `--speech-models` flag followed by each model name as a separate argument. For example:

```bash
assemblyai transcribe ~/audio/my_audio_file.mp3 --speech-models universal-3-pro universal-2
```

<Note>
  The `--speech-models` flag accepts a list of model names separated by spaces,
  not a single string. Each model name should be provided as a separate
  argument.
</Note>
