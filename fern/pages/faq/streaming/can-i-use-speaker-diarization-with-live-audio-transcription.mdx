...
Streaming diarization can be accomplished by using multichannel audio (i.e., an audio stream for each speaker). A separate session is created for each speaker and a single transcript is created from those sessions. Check out [this section](/docs/streaming/multichannel-streams) of our documentation to learn more about this approach.
