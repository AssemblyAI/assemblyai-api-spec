...
- **Universal-Pro model** delivers exceptional accuracy for medical terminology and clinical documentation
...
### Use Pre-recorded (Universal-Pro) When:

**Post-visit documentation** - Encounter already happened, need highest accuracy
- **Maximum accuracy required** - Universal-Pro has highest medical terminology accuracy
...
2. **Universal-Pro post-processing** - Run audio through Universal-Pro after visit for:
...
- Visit ends → Audio sent to Universal-Pro for final high-accuracy transcription
...
### Pre-Recorded Doctor Patient Visits (Universal-Pro)
**Languages:**
For post-visit documentation, Universal-Pro supports English for the highest accuracy transcription. If you want to use other languages, Universal is a suitable alternative.
...
**Recommended approach:** Use streaming for real-time documentation, then run through Universal-Pro post-visit for accurate speaker-labeled final notes.

### Coming Soon
- Medical model which packages up the best ways to contextually influence transcript output
- Multilingual Universal-Pro, especially important for multilingual medical conversations to improve accuracy

## How Can I Get Started Building a Post-Visit Medical Scribe?

Here's a complete example implementing async transcription with Universal-Pro:
...
async def transcribe_encounter_async(audio_source: str) -> Dict:
    ...
    config = aai.TranscriptionConfig(
        # Use Universal-Pro for medical accuracy
        speech_model=aai.SpeechModel.slam,
...
async def generate_clinical_documentation(audio_file: str, patient_id: str):
    ...
    # Step 1: Transcribe with Universal-Pro
    config = aai.TranscriptionConfig(
        speech_model=aai.SpeechModel.slam,
        speaker_labels=True,
        speakers_expected=2,
...
- [Universal-Pro Documentation](https://www.assemblyai.com/docs/speech-to-text/pre-recorded-audio)
