...
parameter: keyterms (array of strings, connection parameter)
description: Improve streaming transcription accuracy for specific words and phrases. Supported on Universal Streaming models only.
*/}
...
After each turn is completed, an additional boosting pass analyzes the full transcript using your keyterms list. This post-processing step provides a second layer of accuracy improvement by examining the complete context of the turn. Turn-level boosting is enabled by default when `format_turns=true`.

<Note>
For Universal-Streaming English, Universal-Streaming Multilingual, and Universal-3 Pro (`u3-rt-pro`), turn-level boosting is active by default. If you set `format_turns=false`, turn-level boosting will be disabled for the Universal-Streaming models.
</Note>
