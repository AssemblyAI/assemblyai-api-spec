...
<Note>
  For Universal-Streaming English and Universal-Streaming Multilingual, you must
  set `format_turns` to `True` to enable turn-level boosting. For Universal-3
  Pro (`u3-rt-pro`), turn-level boosting is always active since formatting is
  built into the model — there is no need to set `format_turns`.
</Note>
