...
- **`utterance`** — A snapshot of the finalized text at the moment an utterance boundary is detected. It is populated **only** on the specific Turn message where an utterance completes — this is useful for pre-emptive LLM generation. On all other Turn messages, including the `end_of_turn: true` message (if it arrives separately) and the formatted message, `utterance` is an empty string `""`.
...
Before any Turn messages are sent, the server sends a `SpeechStarted` message indicating that speech has been detected in the audio stream. The `timestamp` field indicates when the speech was detected, in milliseconds relative to the beginning of the audio stream.

```json
{ "type": "SpeechStarted", "timestamp": 1920 }
```
...
```json
{ "type": "SpeechStarted", "timestamp": 5200 }
```
