...
<Note>
  Because the utterance end and turn end occur on the **same message** here, both `utterance` and `transcript` contain the finalized text. When the utterance ends *before* the turn (see [below](#end-of-utterance-final-transcript-not-end-of-turn)), `utterance` is populated on the earlier message and will be **empty** on the subsequent `end_of_turn: true` message.
</Note>
...
<Note>
  Notice that `utterance` is an empty string in this message. The `utterance` field was already populated on the [previous message](#end-of-utterance-final-transcript-not-end-of-turn) when the utterance boundary was first detected. On the `end_of_turn: true` message, use `transcript` to get the complete turn text.
</Note>
