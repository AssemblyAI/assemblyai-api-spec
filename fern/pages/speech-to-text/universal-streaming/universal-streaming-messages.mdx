...
- Avoid using `format_turns` for voice agent pipelines as it will significantly increase latency. LLMs don't need formatting so you can just pass raw text as soon as it's ready. If you're building a notetaking or closed captioning app where output is displayed to users, set `format_turns=true`.
