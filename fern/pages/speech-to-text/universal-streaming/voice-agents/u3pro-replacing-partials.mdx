---
title: "U3 Pro partial behavior"
description: "How U3 Pro emits partials, finalizes turns, and how to best configure for your use case."
---

## Overview

Traditional streaming models (like Universal-Streaming-English and Universal-Streaming-Multilingual) emit partials word-by-word as audio is processed. Each word can revise until it's marked final, but once final, it never changes.

*Universal-3-Pro* takes a different approach: partials are only emitted during periods of silence, producing stable, fully transcribed segments rather than incremental word-by-word updates.

All words in partials are marked `word.word_is_final=false`.

While the segments are stable, the final end-of-turn transcript may differ from earlier partials as the model refines its output with full turn context.

On the final end-of-turn transcript, all words are marked `word.word_is_final=true`.

## How U3 Pro partials work
...
## Using partials for speculative inference
...
Setting `max_turn_silence` too low can have the same impact, but is less likely to cause entity splitting since it only triggers after a longer silence. If you have a use case with very long (>1s) pauses and you'd like to keep these utterances as a single turn, you may want to increase `max_turn_silence` to avoid cutting off the turn too early.
...
{"type": "UpdateConfiguration", "max_turn_silence": 3000}
```
...
{"type": "UpdateConfiguration", "max_turn_silence": 1000}
```

A clean way to implement this is by giving your LLM a tool call:
```python
DEFAULT_MAX_TURN_SILENCE = 1200  # your preferred default (ms)
EXTENDED_MAX_TURN_SILENCE = 5000  # your preferred extended value (ms)

def dynamically_set_turn_silence(ws, max_turn_silence_ms: int):
    f"""Adjust max_turn_silence on the STT stream.
    Use {EXTENDED_MAX_TURN_SILENCE} when expecting long utterances (credit cards, phone numbers).
    Use {DEFAULT_MAX_TURN_SILENCE} to restore normal turn detection."""
