...
This guide covers integrating AssemblyAI's **Universal-3 Pro (u3-rt-pro)** streaming speech-to-text model into a LiveKit voice agent using the Agents framework. U3-Pro is AssemblyAI's next-generation model built specifically for voice agent use cases, featuring advanced turn detection, prompt engineering capabilities, and optimized latency.

<Note>
  U3-Pro support is not yet live within LiveKit's official plugin. You need to
  install directly from the forked repository below.
</Note>

### Critical configuration requirements

When using U3-Pro with LiveKit, the following settings are essential for optimal performance:

- **Use LiveKit's turn detection, not STT turn detection.** Set `turn_detection=EnglishModel()` (or `MultilingualModel()` for non-English). Do **not** use `turn_detection="stt"` — AssemblyAI's built-in STT turn detection has not been optimized for U3-Pro and will result in poor performance.
- **Enable Silero VAD for interruption handling.** Set `vad=silero.VAD.load()`. This is required because U3-Pro only returns final transcripts (no partials), so interruptions must be detected locally.
- **Set `min_turn_silence` to `100`.** This minimizes end-of-turn latency and prevents "are you there?" moments. The default of `400` adds unnecessary delay with U3-Pro.
- **Set `format_turns` to `True`.** Since U3-Pro only returns final transcripts, formatting adds no latency and should always be enabled.

<Warning>
  Failing to set these parameters correctly is the most common cause of poor
  performance with U3-Pro on LiveKit.
</Warning>
...
Optionally, install the noise cancellation plugin:

```bash
pip install "livekit-plugins-noise-cancellation~=1.0"
```

<Note>
  Noise cancellation is optional and only recommended in very noisy
  environments. It can introduce audio artifacts that negatively impact
  transcription quality. If your telephony provider or SIP trunk already applies
  noise cancellation, adding a second layer can make things worse.
</Note>
...
from livekit.plugins import (
    assemblyai,
    # noise_cancellation,  # Optional: see note below
    silero,
)
...
async def entrypoint(ctx: agents.JobContext):
    ...
    await session.start(
        room=ctx.room,
        agent=Assistant(),
        # room_input_options=RoomInputOptions(  # Optional: enable if needed
...
<Warning>
  Do **not** use `turn_detection="stt"` with U3-Pro. AssemblyAI's built-in STT
  turn detection has not yet been optimized for the U3-Pro model. Using it may
...
<Warning>
  This is critical. Because U3-Pro does not provide partial transcripts,
  interruptions **must** be handled locally via Silero VAD rather than through
  streaming transcription updates. Always keep VAD enabled.
</Warning>
...
<Warning>
  Setting this too low may cause over-segmentation of turns (cutting off the
  user mid-sentence).
</Warning>
...
<ParamField
  ...
  default="100"
>
  Milliseconds of silence before triggering end-of-turn when confident. This is
  the **only endpointing parameter that affects U3-Pro**.
</ParamField>
...
<ParamField path="max_turn_silence" type="int" default="1280">
  Maximum silence (ms) before triggering end-of-turn. **Not applicable to
  U3-Pro.**
</ParamField>
...
| ---------------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------- |
| "Hello, are you there?" from users | High end-of-turn latency                         | Set `min_turn_silence=100`                          |
...
| Turn over-segmentation             | `min_turn_silence` too low | Increase from `100` to `200`-`300` and test                               |
