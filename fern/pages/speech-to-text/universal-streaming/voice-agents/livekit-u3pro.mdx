...
Based on extensive testing (including telephony/SIP environments, UK-accented English, and short utterance scenarios), the following is the recommended configuration for U3-Pro on LiveKit:
...
- **Call out known problem words**: if specific short utterances (e.g., "tonight") are being misrecognized, mention them explicitly
...
- **Key terms boosting**: append boosted words to the end of your prompt:

```python
word_boost = ["tonight", "tomorrow", "Hilton", "Marriott"]
prompt = f"""...your prompt...
Make sure to boost the words {', '.join(word_boost)} in the audio."""
```

<Note>
The `keyterms_prompt` parameter was not properly configured in the initial LiveKit fork. Use the prompt-based approach above for key term boosting instead.
</Note>
...
| Key terms not being applied | `keyterms_prompt` not configured in fork | Use prompt-based boosting instead |
