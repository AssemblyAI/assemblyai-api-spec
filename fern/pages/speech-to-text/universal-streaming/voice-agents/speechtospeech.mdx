...
<Warning>
Do not send `input.audio` before `session.ready` fires. Sending audio before the session is established causes a flood of `session.error` events with code `invalid_format`.
</Warning>
...
Stream PCM16 audio to the agent. Only send this after `session.ready` has fired.
...
Manually trigger a response. Useful when VAD is disabled or you want to force a turn.
...
Cancel the agent's current response mid-speech.
...
| Field      | Type   | Description                                       |
| ---------- | ------ | ------------------------------------------------- |
| `reply_id` | string | From the `reply.started` event                    |
...
Agent has begun generating a response. Save `reply_id` to cancel if needed.
...
Agent has finished speaking. If `status` is `"interrupted"`, the user barged in — discard any pending tool results.
...
When the user speaks mid-response, the server sends `input.speech.started` and automatically stops the agent. The `reply.done` event follows with `status: "interrupted"`.

When an interruption happens:
1. Discard any `pending_tools` accumulated during the interrupted reply
2. Clear the playback buffer if needed
3. The agent is ready to listen again

You can also cancel the current reply manually using the `reply_id` from `reply.started`:
