...
| ------ | -------- | ------ | ------------------- |
| **Strict punctuation rules** (default) | **88%** | **100%** | **24%** |
| No prompt | 83% | 100% | 34% |
| Turn-signal framing | 80% | 98% | 38% |

The strict punctuation rules prompt—which is the [default prompt](#default-prompt) used when no `prompt` is provided—dropped the false positive rate from 34% to 24% compared to no prompt at all. That translates to fewer false interruptions in production voice agent conversations.

**Key takeaways for prompt design:**

1. **The default prompt already optimizes for turn detection.** Without any custom prompt, Universal-3-Pro still achieves 83% accuracy thanks to its built-in punctuation behavior—but the default prompt improves this further. Try the default before customizing.
2. **Concrete punctuation rules outperform semantic turn-taking instructions.** A prompt explicitly framed around "turn signals" and "turn detection" performed worse than one with specific punctuation rules. The model responds better to concrete formatting instructions than abstract conversational concepts.
