...
<Warning title="Entity splitting (accuracy) vs Model Latency trade-off">
  ...
  like phone numbers and emails in half. We have found LLM steps fix this for
  voice agents, but we recommend testing carefully with your use case.
</Warning>
...
Updating prompts can be useful to pass updated context from the agent session into the STT stream via the `prompt` parameter to improve accuracy of the overall transcription.
