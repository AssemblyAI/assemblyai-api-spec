...
- **`transcript`** — The running transcript of the current turn. Use this field for displaying live captions or getting the full turn text.
- **`utterance`** — A snapshot of the finalized text when the turn ends. For Universal-3 Pro, since utterance end and turn end always occur together (turns end on terminal punctuation), `utterance` and `transcript` will contain the same text on the `end_of_turn: true` message. On partial transcripts, `utterance` is always an empty string `""`.

<Note>
  **Key takeaway:** To get the complete text of a turn, always read `transcript`. The `utterance` field is provided for consistency with Universal Streaming, where utterance boundaries can occur independently of turn boundaries.
</Note>

Universal-3 Pro handles message sequences differently from [Universal Streaming](/docs/streaming/message-sequence). Instead of emitting word-by-word partial transcripts as audio is processed, Universal-3 Pro produces transcripts only during silence periods. Key differences include:
