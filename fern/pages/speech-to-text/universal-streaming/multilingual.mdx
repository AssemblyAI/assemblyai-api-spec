...
<Note title="Need more than 6 languages?">
  If you need support beyond the 6 languages listed here, consider using the **Whisper Streaming** model (`speech_model: "whisper-streaming"`), which supports **99 languages** with automatic language detection. See the [Model selection](/docs/streaming/select-the-speech-model#whisper-streaming) page for details.
</Note>
