...
<Note title="Tuning tips">
  - **Monologues and long-form speech:** Increase `min_turn_silence` and `max_turn_silence` to avoid premature turn endings during natural pauses. The Conservative preset is a good starting point, but you may need higher values (e.g., `max_turn_silence: 5000` or more) for extended monologues where speakers pause mid-thought.
  - **Rapid dialogues:** Decrease both values to end turns quickly and reduce latency. The Aggressive preset works well here.
  - **Mixed conversation patterns:** Start with the Balanced preset and adjust incrementally. If you see turns ending on incomplete sentences, increase `min_turn_silence` by 100-200ms at a time.
  - **Premature end-of-turn:** If the model ends turns on incomplete sentences, this is a known edge case, especially during long monologues. Raising `min_turn_silence` and `max_turn_silence` reduces the likelihood. You can also use `ForceEndpoint` for explicit turn control when your application logic can determine turn boundaries.

  For detailed examples of entity splitting tradeoffs and latency tuning, see the [tuning endpointing parameters](/docs/streaming/livekit-u3-rt-pro#tuning-endpointing-parameters) and [latency vs. entity splitting trade-off](/docs/streaming/u3-pro-partials#latency-vs-entity-splitting-trade-off) sections.
</Note>
