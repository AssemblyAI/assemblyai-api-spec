...
You must explicitly terminate streaming sessions when you are done transcribing. If a session is not explicitly terminated, it remains open and continues to accrue billing charges until the server automatically closes it after 3 hours (error code `3008`).

To terminate a session, use one of the following methods:
...
After sending the `Terminate` message, the server responds with a `Termination` message containing `audio_duration_seconds` and `session_duration_seconds`, and then closes the WebSocket connection.

<Warning>
  Simply closing the WebSocket connection without sending a `Terminate` message
  does not guarantee the session ends immediately. Always send an explicit
  termination to ensure the session is properly closed and billed only for the
  audio you processed.
</Warning>
