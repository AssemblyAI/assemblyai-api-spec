...
<Tip title="Already using AssemblyAI streaming?">
  ...
  your connection parameters. You should also set `min_turn_silence` and
  `max_turn_silence` to tune turn detection for U3-Pro — we recommend
  `min_turn_silence=100` and `max_turn_silence=1200` as a starting point. No
  other code changes are required — just update the model, set your turn silence
  parameters, and start streaming.
</Tip>
...
<Warning title="Prompting: Beta">
  Prompting for streaming is currently in beta. Prompting behavior may change as
  we continue to refine the model. We recommend testing thoroughly and iterating
  on your prompts as the feature evolves.
</Warning>
...
<Note title="Prompt and Keyterms Prompt">
  ...
  prompt](#default-prompt) automatically. See [How keyterms prompting works
  behind the scenes](#how-keyterms-prompting-works-behind-the-scenes) below.
</Note>

### How keyterms prompting works behind the scenes

Behind the scenes, the `keyterms_prompt` parameter works by appending your keyterms to the default prompt. The model receives the combined `<prompt> + <keyterms>` as a single input, which is how keyterms are boosted during transcription.
...
Make sure to boost the words Kelly Byrne-Donoghue, AssemblyAI, cardiology in the audio.
```
