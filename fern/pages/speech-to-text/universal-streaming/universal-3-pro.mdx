...
If you're an existing AssemblyAI streaming user, you can quickly test Universal-3 Pro by switching the `speech_model` parameter to `"u3-pro"` in your connection parameters. No other code changes are required — just update the model and start streaming.
</Tip>
...
The `prompt` and `keyterms_prompt` parameters cannot be used in the same request. Please choose either one or the other based on your use case. When you use `keyterms_prompt`, your boosted words are appended to the [default prompt](#default-prompt) automatically.
</Note>
