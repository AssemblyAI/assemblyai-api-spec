...
If you're going to build a prompt, start with the [default prompt](#default-prompt) and then tweak it for your use case. **You should not start from scratch with your prompt — use the default prompt and then build off of it.** See the [Prompting Guide (Streaming)](/docs/universal-streaming/prompting) if you'd like to build your prompt yourself.

Remember, prompts are primarily instructional, so adding a large amount of context may not make a significant impact on accuracy and could reduce instruction-following coherence. Feel free to layer in additional instructions that you see in the [Prompting Guide (Streaming)](/docs/universal-streaming/prompting).

Universal-3-Pro also supports `keyterms_prompt` for boosting specific terms. See [Keyterms prompting](/docs/universal-streaming/keyterms-prompting) for details.
...
You can set `keyterms_prompt` at connection time or [update it mid-stream](#updating-configuration-mid-stream) as the conversation progresses. For full details, see [Keyterms prompting](/docs/universal-streaming/keyterms-prompting).
...
You can override the default prompt by providing your own `prompt` value. For detailed guidance on crafting effective prompts for streaming, see the [Prompting Guide (Streaming)](/docs/universal-streaming/prompting).
...
- **`keyterms_prompt`** — Dynamically add terms relevant to the current stage of your voice agent flow. This is the most effective way to improve recognition accuracy mid-stream. See [Keyterms prompting](/docs/universal-streaming/keyterms-prompting) for details.
