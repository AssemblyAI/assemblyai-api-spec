...
<Warning title="Streaming Diarization: Public beta">
  Streaming Diarization is currently in public beta. We are actively scaling
  infrastructure and tuning the model. You can start building and testing with
  it today, but be aware that behavior may change as we continue to improve the
  experience.
</Warning>

Streaming Diarization lets you identify and label individual speakers in real
time directly from the Streaming API. Each `Turn` event includes a `speaker`
field (e.g. `SPEAKER_A`, `SPEAKER_B`) indicating which speaker produced that
...
<Tip title="Already using AssemblyAI streaming?">
  ...
  connection parameters. No other changes are required — the `speaker` field
  will appear on every `Turn` event automatically.
</Tip>
...
When diarization is enabled, every `Turn` event includes a `speaker` field
with a label such as `SPEAKER_A`, `SPEAKER_B`, and so on.

```json
{
  "type": "Turn",
  "transcript": "And then after that, they had a phone relationship.",
  "speaker": "SPEAKER_A",
  "end_of_turn": true,
...
utterances at the start of a session — the `speaker` field may be `null` or
`"UNKNOWN"`. Your application should handle this case gracefully.

A typical multi-speaker exchange looks like this:

```
[SPEAKER_A] And then after that, they had a phone relationship, and finally they actually met.
[SPEAKER_A] God. He was from someplace like Boston or something.
[SPEAKER_B] So you never know.
[SPEAKER_B] Yeah.
[SPEAKER_A] Do you have friends, Laura?
[SPEAKER_C] Sort of. I have the midwives, and two of them live here, and they're friendly.
```
...
Real-time diarization is an inherently harder problem than offline diarization.
