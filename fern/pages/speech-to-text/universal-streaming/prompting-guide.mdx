...
Universal-3-Pro also supports `keyterms_prompt` for boosting specific terms. We suggest using this with no prompt. The `prompt` and `keyterms_prompt` parameters cannot be used in the same API request — choose one or the other. However, you can recreate this functionality manually. See [Keyterms prompting](#keyterms-prompting) below.
...
<Note title="Prompt and Keyterms Prompt">
  ...
  prompt](#start-with-the-default-prompt) automatically. See [How keyterms
  prompting works behind the
  scenes](#how-keyterms-prompting-works-behind-the-scenes) below.
</Note>
