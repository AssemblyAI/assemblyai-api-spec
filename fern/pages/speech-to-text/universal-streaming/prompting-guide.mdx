...
<Note title="Looking for async prompting?">
...
Providing topic context alone (e.g., "this is a cardiology appointment") can help, but is most effective when paired with specific instructions telling the model *how* to transcribe. For domain-specific term boosting, use the [`keyterms_prompt`](/docs/universal-streaming/keyterms-prompting) parameter to explicitly list the terms you want the model to recognize.
...
You can set `keyterms_prompt` at connection time or [update it mid-stream](/docs/universal-streaming/universal-3-pro#updating-configuration-mid-stream) as the conversation progresses. For full details, see [Keyterms prompting](/docs/universal-streaming/keyterms-prompting).

<Note title="Prompt and Keyterms Prompt">
The `prompt` and `keyterms_prompt` parameters cannot be used in the same request. Please choose either one or the other based on your use case. When you use `keyterms_prompt`, your boosted words are appended to the [default prompt](#start-with-the-default-prompt) automatically.
</Note>
