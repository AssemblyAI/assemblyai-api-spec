...
The current system prompt as of February 25, 2026 is:
...
Prior to February 25, 2026 the system prompt was:
...
<Warning title="Avoid unintended speaker attribution">
...
Speaker diarization and speaker identification are stable, consistent models whereas speaker attribution via prompting is experimental and may produce inconsistent results, especially across longer files where the model processes audio in chunks. **Using the word `speaker` anywhere in your prompt, even in unrelated context, can trigger inline speaker attribution labels** (for example, `[Speaker:NAME]`) embedded directly in the transcript text. Avoid the word `speaker` in your prompt unless you specifically want this behavior.

For production use cases requiring consistent speaker labels, use the [`speaker_labels`](/docs/pre-recorded-audio/speaker-diarization) parameter or the [speaker identification](/docs/speech-understanding/speaker-identification) feature instead of prompting. In the future we will be natively building the model's capabilities here into these features.
