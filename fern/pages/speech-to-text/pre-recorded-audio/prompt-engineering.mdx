...
Prompting behavior differs between async (pre-recorded) and streaming use cases. This guide covers prompting for **async (pre-recorded audio)**. If you're working with real-time audio, see the [Prompting Guide (Streaming)](/docs/universal-streaming/prompting).
</Note>
...
Prompting is **instructional** — the model responds to explicit formatting rules and behavioral instructions (e.g., "include all filler words" or "use periods only for complete sentences") rather than open-ended topic context alone. Providing domain context like "this is a cardiology appointment" is most effective when paired with specific instructions telling the model *how* to transcribe. For boosting specific domain terms, use [keyterms prompting](/docs/pre-recorded-audio/keyterms-prompting).
...
This is the current default prompt, providing the best balance of verbatim accuracy, multilingual support, and performance on challenging audio:
...
<Tip>
For a more robust approach to handling unclear audio, see the [Handling unclear audio with [masked]](#handling-unclear-audio-with-masked) section in Recommended prompts above. The `[masked]` strategy provides explicit flagging of uncertain segments and materially reduces hallucinations.
</Tip>
...
- **Watch for semantic substitutions** — some substitutions are purely semantic or formatting differences (e.g., "offsite" vs. "off site," "alright" vs. "all right") that inflate WER without representing meaningful errors.
