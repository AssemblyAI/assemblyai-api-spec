...
```text
Label speakers by role when identifiable (Judge:, Petitioner's Counsel:).
```
...
**Caution:** Speaker attribution generated by the model are in addition to the speaker diarization feature. We recommend using one or the other. Speaker diarization is a stable, consistent model whereas speaker attribution is experimental and may give inconsistent results. Using the word `speaker` anywhere in your prompt will generate labels so avoid this word to ensure this capability is not activated.
...
| **Explicit disfluency instructions** | High    | `Include all disfluencies, filler words, and hesitations`                                                                    | Model sees the speech patterns and linguistic cues to pay extra attention to         |
...
Required: Capture every hesitation, false start, and filler word as spoken.
...
and filler words exactly as spoken.
...
and thinking sounds exactly as spoken.
...
Resolve sound-alike errors using bilingual context.
```
...
hesitations, restarts, and self-corrections.
```
