...
Non-negotiable: Capture Latin legal phrases exactly as spoken (certiorari,
habeas corpus, amicus curiae) and preserve case citations and procedural language.

Label participants by role when identifiable (judge, counsel, witness).
```

**Why it works:** Combines authoritative language (Mandatory, Required, Non-negotiable), explicit disfluency examples, specific domain terminology with Latin phrases listed, and role-based participant labeling.
...
Non-negotiable: Preserve financial terminology exactly as spoken including
EBITDA, year-over-year, and basis points with standard notation.

Format numerical data with standard notation. Label executives by role
...
Non-negotiable: Preserve technical terminology exactly as spoken including
Kubernetes, PostgreSQL, and API with correct capitalization.

Mark transitions between participants explicitly. Capture self-corrections
and restarts (I was— I went, th-that).
```
...
Non-negotiable: Pay close attention to proper nouns, balance amounts, and bank
names being transcribed correctly.
```
