...
<Note title="Prompts only apply to Universal-3-Pro">
  The `prompt` parameter is supported by Universal-3-Pro only. If you use
  multi-model routing with `speech_models` (e.g., `["universal-3-pro",
  "universal-2"]`) and the system falls back to Universal-2 for an unsupported
  language, your prompt will not be applied to that request.
</Note>
...
In addition to the [default prompt](#best-all-around-default), we recommend testing with the [Best simple prompt](#best-simple) and the [Handling unclear audio with [masked]](#handling-unclear-audio-with-masked) prompt to find the best fit for your evaluation and use case.
...
<Note>
  ...
  as a workaround using the `Context:` prefix. For example: `Mandatory: Pay
  close attention to medications and diseases and be obsessively accurate.
  Context: omeprazole, metformin, hypertension.`
</Note>
