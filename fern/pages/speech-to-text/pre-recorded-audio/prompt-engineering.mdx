...
### Prompting for streaming: turn detection

For streaming use cases, Universal-3 Pro includes turn detection built directly into the model. Turn detection determines when a speaker has finished their turn, which is critical for voice agent workflows and real-time conversation systems.

Two key parameters control turn detection behavior:

- **`min_end_of_turn_silence_when_confident`** — Sets the silence duration (in milliseconds) before a speculative end-of-turn check is triggered.
- **`max_turn_silence`** — Sets the maximum silence duration (in milliseconds) before a turn is forcibly ended.

Because turn detection is punctuation-based, prompting directly influences when turns end. The model's default streaming prompt is already optimized for turn detection accuracy. For detailed guidance on configuring turn detection and optimizing prompts for streaming, see the [streaming turn detection configuration](/docs/universal-streaming/universal-3-pro#configuring-turn-detection) and [streaming prompting guide](/docs/universal-streaming/prompting).
