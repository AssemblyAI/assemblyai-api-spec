...
```text
Mandatory: Transcribe legal proceedings with precise legal terminology intact.

Required: Preserve all disfluencies including verbal pauses, false starts, and
self-corrections (uh, um, I—I mean) exactly as spoken.

Non-negotiable: Accuracy of legal terminology including Latin phrases,
case citations, and procedural language.

Label participants by role when identifiable (judge, counsel, witness).
```

**Why it works:** Combines authoritative language (Mandatory, Required, Non-negotiable), explicit disfluency examples, domain terminology categories, and role-based participant labeling.
...
Required: Capture every hesitation, false start, and filler word
(um, uh, ah) as spoken.

Context: a medical consultation discussing medications and symptoms.
Prioritize accurately transcribing pharmaceutical names, diseases, and
treatment plans wherever possible.
```

**Why it works:** Combines authoritative language (Mandatory, Required) with explicit disfluency examples and domain context paired with actionable instructions for clinical accuracy.
...
Required: Preserve all disfluencies including hesitations, false starts,
and filler words (um, uh, you know) exactly as spoken.

Non-negotiable: Accuracy of financial terminology including acronyms,
metrics, and standard financial notation.

Format numerical data with standard notation. Label participants by role
when identifiable (executive, analyst, moderator).
```

**Why it works:** Balances financial terminology precision with verbatim capture of speech patterns, using pattern-based entity accuracy instructions instead of specific corrections.
...
Non-negotiable: Accuracy of technical terminology including framework names,
programming languages, and acronyms.

Capture self-corrections and restarts (I was— I went, th-that).
```

**Why it works:** Preserves natural developer speech patterns while using pattern-based instructions for technical terminology accuracy.
...
Required: Preserve all disfluencies exactly as spoken including verbal
hesitations, restarts, and self-corrections (um, uh, I—I mean).

Non-negotiable: Accuracy of proper nouns, financial amounts, and
institutional names.
```
