...
<Warning title="speech_models must be a list">
  The `speech_models` parameter must be a JSON array (e.g., `["universal-2"]`),
  not a string. Passing a stringified array like `"[\"universal-2\"]"` will
  result in a validation error. Make sure you pass an actual list in your SDK or
  a JSON array in your API request.
</Warning>
