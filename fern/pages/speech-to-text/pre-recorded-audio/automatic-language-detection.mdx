...
<Note title="When to use manual language selection">
  Automatic language detection determines the dominant language based on initial audio segments, which means early content heavily influences the result. If the beginning of your audio contains a different language, accent, or non-speech audio, detection may select the wrong language for the entire transcript. When you know the primary language in advance, set `language_code` manually instead of relying on automatic detection. Use automatic detection only when the language is truly unknown. You can also use `expected_languages` to restrict detection to a specific set of candidate languages and reduce misdetection.
</Note>
