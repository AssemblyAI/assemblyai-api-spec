...
To manually set the [language codes](/docs/pre-recorded-audio/code-switching-docs), you can use the `language_codes` parameter. A max of two language codes can be set and one code must be `"en"`. For example, if your file contains both English and Spanish, it would be `"language_codes": ["en", "es"]`.
