...
<ParamField path="language" type="string" required={false} default={"en"}> 
  The language code for the audio stream. If not specified, defaults to `en`.
  Allowed values: `en`, `multi`.
...
<ParamField path="filter_profanity" type="boolean" default={"False"}>
  Whether to filter profanity from the transcribed text, can be true or false.
</ParamField>
