...
- `language_code`: The ISO 639-1 language code of the detected language (e.g., `"es"` for Spanish, `"fr"` for French)
...
In this example, the model detected Spanish (`"es"`) with a confidence of 0.999997 (99.9997%).
                    ...
                    if data['type'] == 'Turn':
                        if data.get('turn_is_formatted'):
                            ...
                            if 'language_code' in data:
                                lang_info = f" [{data['language_code']} - {data['language_confidence']:.2%}]"
                                print(f"\r{transcript}{lang_info}")
                            else:
