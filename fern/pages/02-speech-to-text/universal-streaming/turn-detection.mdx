...
- Set a VAD-based silence latency for each turn: Set `end_of_turn_confidence_threshold` to 1. This will cause the model to end a turn after a pre-determined amount of silence (based on `max_turn_silence`). Most useful when you are using the model as your VAD for silence-based turns.
- Return turns as fast as possible on silence: Set `end_of_turn_confidence_threshold` to 0. This will cause the model to force end-of-turn as soon as silence is detected (based on `min_end_of_turn_silence_when_confident`). Most useful when you are using a custom turn detection model on top of the transcript results.
