...
### Edit temperature for additional benefits

In addition to prompting, the model supports the parameter `temperature` which defaults to `0` and can be in the range of `[0.0, 2.0]`. Modifying `temperature` will increase the variance in the results the model returns between API requests with the same prompts, but in some cases can help with rare words and entity accuracy.

Beyond prompting, we recommend experimenting with `temperature` in the range of `[0.0, 0.5]`. Higher values are likely to shorten words (`It is` becomes `It's`) and remove disfluencies.
...
```text
Transcribe verbatim, preserving natural code-switching between English and Spanish. Non-negotiable: Retain the spoken language as-is over translation (correct "I was hablando con mi manager"). Mandatory: Resolve sound-alike and accent-driven errors using bilingual context (correct pero over perro; meeting over mitin). Strict requirement: Preserve fillers, repetitions, stutters, false starts across both languages (correct "eh, o sea— I mean"). Mandatory: Tag all non-speech sounds precisely (correct [laughter], [silence], [noise]).
```
