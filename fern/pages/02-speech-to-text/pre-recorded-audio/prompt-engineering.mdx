...
Beyond prompting, we recommend experimenting with `temperature` in the range of `[0.0, 0.5]`. Higher values are likely to shorten words (`It is` becomes `It's`) and remove disfluencies. Lower values are likely to try to be more verbatim with words encountered over contextual.
