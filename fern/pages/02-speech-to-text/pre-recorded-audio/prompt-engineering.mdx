...
Beyond prompting, we recommend experimenting with `temperature` in the range of `[0.0, 0.3]`. Higher values are likely to shorten words (`It is` becomes `It's`) and remove disfluencies. Lower values are likely to try to be more verbatim with words encountered and based on phonetics over contextual correctness in the sentence.

Our testing has shown that prompting provides more value than temperature. We recommend working with your prompts first before you experiment with temperature.
