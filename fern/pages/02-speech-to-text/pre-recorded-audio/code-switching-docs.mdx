...
Supports code switching between 99 languages, with best performing language pairs being `en`, `es`, `de`.
...
```python title="Python SDK" highlight={10-13} maxLines=15
...
```javascript title="JavaScript SDK" highlight={15-19} maxLines=15
...
```csharp title="C#" highlight={62-67} maxLines=15
using System;
using System.IO;
...
config = aai.TranscriptionConfig(language_codes=["en", "es"]) # English-Spanish code switching
...
class Program
{
    ...
    static async Task<Transcript> CreateTranscriptAsync(string audioUrl, HttpClient httpClient)
    {
        ...
        {
            audio_url = audioUrl,
            language_codes = new[] { "en", "es" }  // English-Spanish code switching
        };
...
$data = array(
    "audio_url" => $upload_url,
    "language_codes" => ["en", "es"] // Spanish for English-Spanish code switching
);
...
```javascript title="JavaScript SDK" highlight={18} maxLines=15
import { AssemblyAI } from "assemblyai";
...
```csharp title="C#" highlight={66} maxLines=15
using System;
using System.IO;
using System.Net.Http;
using System.Net.Http.Headers;
