...
We use a combination of models to produce your results. With the optional `speech_models` parameter, you can list multiple speech models in priority order from left to right, allowing our system to automatically route your audio to the best available option. You can visit our <a href="https://www.assemblyai.com/pricing" target="_blank">pricing page</a> for more information on our model tiers.

**Model routing behavior:** The system uses the order of models in the list to determine priority, with the first (leftmost) model having the highest priority. For example, with `["slam-1", "universal"]`, the system will try to use Slam-1 first, but fall back to Universal for non-English audio since Slam-1 only supports English.
...
# Excluding the `speech_models` parameter defaults to the Universal model.
transcript = aai.Transcriber().transcribe(audio_file)

# To specify a different model or multiple models, comment out line 11 and uncomment the following two lines to set the `speech_models` parameter.
...
data = {
    ...
    # Excluding the `speech_models` parameter defaults to the Universal model.
    # To specify a different model or multiple models, uncomment the line below to set the `speech_models` parameter.
