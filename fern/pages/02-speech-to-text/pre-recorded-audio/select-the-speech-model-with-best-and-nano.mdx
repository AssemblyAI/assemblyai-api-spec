...
**Model routing behavior:** The system uses the priority order to determine which model to use based on the request characteristics. For example, with `["universal-pro", "universal"]`, the system will try to use universal-pro first, but fall back to universal for non-English audio since universal-pro only supports English
...
| ----------------------- | --------------------------- | ------------------------------------------------------------------------------------- |
| **Universal 3** (default) | `aai.SpeechModel.universal` | Use our fastest, most robust models with the broadest [language support](/docs/speech-to-text/pre-recorded-audio/supported-languages).          |
| **Universal 3 Pro**      | `aai.SpeechModel.universal-pro`    | Use our most customizable model for your transcription (English only).                |
...
| Name                    | SDK Parameter | Description                                                                           |
| ----------------------- | ------------- | ------------------------------------------------------------------------------------- |
| **Universal 3** (default) | `['universal']` | Use our fastest, most robust models with the broadest [language support](/docs/speech-to-text/pre-recorded-audio/supported-languages).          |
| **Universal 3 Pro**       | `['universal-pro']`    | Use our most customizable model for your transcription (English only).                |
...
| ----------------------- | ---------------------------- | ------------------------------------------------------------------------------------- |
| **Universal 3** (default) | `"speech_models":["universal"]` |  Use our fastest, most robust models with the broadest [language support](/docs/speech-to-text/pre-recorded-audio/supported-languages)          |
| **Universal 3 Pro**      | `"speech_models": ["universal-pro"]`    | Use our most customizable model for your transcription (English only).                |
...
You can change the model by setting the `speech_models` in the transcription config:
...
# By default our Universal model is used, comment in the code below if you would like to use our Slam-1 model
# config = aai.TranscriptionConfig(speech_models=aai.SpeechModel.slam_1)

# Default: Universal model
transcript = aai.Transcriber().transcribe(audio_file)

# Optional: To use Slam-1 model instead, uncomment both lines below:
# config = aai.TranscriptionConfig(speech_models=aai.SpeechModel.slam_1)
...
```python highlight={20} maxLines=15
import requests
import time
...
```javascript highlight={16} maxLines=15
...
```javascript highlight={20} maxLines=15
...
```csharp highlight={70} maxLines=15
using System;
using System.IO;
