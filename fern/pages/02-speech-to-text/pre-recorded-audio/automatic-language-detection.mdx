...
```python title="Python SDK" highlight={8-11,13} maxLines=15
...
```python title="Python" highlight={19-23} maxLines=15
import requests
import time
...
```javascript title="JavaScript SDK" highlight={12-16} maxLines=15
...
```javascript title="JavaScript" highlight={19-23} maxLines=15
...
```csharp title="C#" highlight={69-74}  maxLines=15
using System;
using System.IO;
...
```ruby title="Ruby" highlight={23-27} maxLines=15
require 'net/http'
require 'json'
...
```php title="PHP" highlight={30-34} maxLines=15
<?php
$ch = curl_init();
...
If you're confident the audio is in one of a few languages, provide that list via `language_detection_options.expected_languages`. Detection is restricted to these candidates and the model will choose the language with the highest confidence from this list. This can eliminate scenarios where .

- Use our [languages](/docs/speech-to-text/pre-recorded-audio/supported-languages) (e.g., `"en"`, `"es"`, `"fr"`).
- To allow all supported languages, set `expected_languages` to `"all"`.

<Note>
  In order to not introduce a breaking change to our API,
  `language_detection_options.expected_languages` currently defaults to our
  previous 17 supported lanugages for Automatic Language Detection on Universal.
  These are
  `["en","es","fr","de","hi","it","ja","pt","nl","zh","vi","ru","tr","ko","fi","pl","uk"]`
</Note>
...
```python title="Python" highlight={21} maxLines=15
import requests
import time
...
```javascript title="JavaScript SDK" highlight={14} maxLines=15
import { AssemblyAI } from "assemblyai";
...
```javascript title="JavaScript" highlight={21} maxLines=15
import axios from "axios";
import fs from "fs-extra";
...
```csharp title="C#" highlight={72}  maxLines=15
using System;
using System.IO;
using System.Net.Http;
using System.Net.Http.Headers;
...
```ruby title="Ruby" highlight={25} maxLines=15
require 'net/http'
require 'json'
...
```php title="PHP" highlight={32} maxLines=15
<?php
$ch = curl_init();
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
- If `expected_languages` is provided and `fallback_language` is omitted, the default fallback is English (`"en"`).
...
<Note>
  In order to not introduce a breaking change to our API,
  `language_detection_options.fallback_language` currently defaults to `en`.
</Note>
...
```python title="Python" highlight={22} maxLines=15
import requests
import time
...
```javascript title="JavaScript SDK" highlight={15} maxLines=15
import { AssemblyAI } from "assemblyai";
...
```javascript title="JavaScript" highlight={22} maxLines=15
import axios from "axios";
import fs from "fs-extra";
...
```csharp title="C#" highlight={73}  maxLines=15
using System;
using System.IO;
using System.Net.Http;
using System.Net.Http.Headers;
...
```ruby title="Ruby" highlight={26} maxLines=15
require 'net/http'
require 'json'
...
```php title="PHP" highlight={33} maxLines=15
<?php
$ch = curl_init();
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
