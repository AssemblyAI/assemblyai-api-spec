---
title: "(Early Access) Automatic Language Detection"
---

<Tip title="Early Access">
  This Automatic Language update is currently in Early Access. During this Early
  Access period, you will need to use the `expected_languages` parameter with a
  value of `["all"]` for ALD to detect all 99 of our supported languages. If you
  only use `language_detection` with a value of true during Early Access, it
  will only detect between the 17 currently supported languages. After the
  official launch on August 26th, 2025 using `language_detection` with a value
  of `"true"` will detect all 99 languages supported by our Universal model.
</Tip>
...
```python title="Python SDK" for="python-sdk" highlight={8-11,13} maxLines=15
...
```python title="Python" for="python" highlight={19-23} maxLines=15
import requests
import time
...
```javascript title="JavaScript SDK" for="javascript-sdk" highlight={12-16} maxLines=15
...
```javascript title="JavaScript" for="javascript" highlight={19-23} maxLines=15
...
```csharp title="C#" for="csharp" highlight={69-74}  maxLines=15
using System;
using System.IO;
...
```ruby title="Ruby" for="ruby" highlight={23-27} maxLines=15
require 'net/http'
require 'json'
...
```php title="PHP" for="php" highlight={30-34} maxLines=15
<?php
$ch = curl_init();
...
- To allow all supported languages, set `expected_languages` to `["all"]`.
...
- Set `fallback_language` to `"auto"` to let our model choose the fallback language from `expected_languages` with the highest confidence score.
