...
[Universal-Streaming API: Message Sequence Breakdown](/docs/speech-to-text/universal-streaming-messages-guide)
...
- Use a sample rate of 16 kHz and encoding of pcm_s16le. 8 kHz or 48 kHz are workable, but may result in latency fluctuations or increased latency
...
End-of-turn functionality combines model-based semantic analysis with acoustic heuristics to detect when a user has finished speaking. Both mechanisms work jointly and either can trigger end-of-turn detection throughout the audio stream.

<Note> End-of-turn and end-of-utterances refer to the same thing and may be used interchangeably in these docs.</Note>
...
* Acoustic detection can override model-based detection even with high EOT confidence thresholds
...
In the current iteration of our streaming model, "Partial transcripts" are **immutable**, unchanging outputs. They are delivered in the following format:
...
This is followed by a Final Transcript:
...
<Note>
  To optimise for latency, we recommend using the unformatted Final Transcript
  as it's received more quickly than the formatted version. However, this
  creates a tradeoff as you won't receive punctuation and proper casing. Choose
  based on whether speed or formatting is more important for your application.
</Note>

### Implementation strategy

Since our Partial Transcripts are immutable, the data is immediately ready to be sent in the voice agent pipeline. Here's how to handle the data flow:

1. When you receive a Final Transcript but your Voice Agent hasn't detected end of turn, save this data in a variable (let's call it `running_transcript`).
...
3. Clear the `running_transcript` after sending and be sure to ignore the Final Transcript that will eventually arrive for the latest partial you used. This prevents duplicate information from being processed in future turns.

What you send to the voice agent should look like:
`running_transcript` + ' ' + `latest_partial`
...
Utilizing our Partial Transcripts in this manner will allow you to achieve the fastest possible latency for this step of your Voice Agent. Please reach out to the AssemblyAI team with any questions.

<Note>
  When using the latest Partial Transcript, ignore the subsequent Final
  Transcript for that segment to avoid sending duplicate information to the
  voice agent.
</Note>
...
```json
{ "type": "UpdateConfiguration", "word_finalization_max_wait_time": 5, "end_of_turn_confidence_threshold": 0.1 }
```
