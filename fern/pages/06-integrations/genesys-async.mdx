...
You may already have an inbound call flow set up in Genesys (if so, skip to [Step 3](https://www.assemblyai.com/docs/integrations/genesys-cloud#step-3-create-a-s3-bucket)), but we'll create a simple one from scratch for the sake of this tutorial.
...
In the Genesys Cloud Admin section, navigate to the [Phones page](https://apps.usw2.pure.cloud/directory/#/admin/telephony/phone-management/phones) under the Telephony section. Create a new phone and assign the User from your organization that you selected for the **Transfer to User** step in the previous section.

</Step>
<Step>

Navigate to the [Number Management](https://apps.usw2.pure.cloud/directory/#/admin/telecom/numbers/numbers) page under the Genesys Cloud Voice section and select **Purchase Numbers**. Create a new phone number.

</Step>
<Step>

Navigate to the [Call Routing](https://apps.usw2.pure.cloud/directory/#/admin/routing/ivrs) page under the Routing section and select **Add**. Under **What call flow should be used?** select your flow. For **Inbound Numbers**, type the number you purchased in the above step.

</Step>
<Step>

Under the Telephony section, navigate to the [Trunks](https://apps.usw2.pure.cloud/directory/#/admin/telephony/trunks/external) page. Create a new external trunk. Under Caller ID, the **Caller Address** will be an E.164 number and the phone number you created. 

</Step>
<Step>

Under SIP Access Control, select **Allow All** (*note: this is only for development and testing purposes, please specify actual IPs in production*). Under the **Media** section, make sure you select **Record calls on this trunk**. Then save your trunk (it may take a few moments for your trunk to be ready.)
...
Click **Create bucket**. Give your bucket a name like `your-audiohook-bucket`. Scroll down to click **Create bucket**.
...
The AudioHook Monitor will send requests to a WebSocket URL we specify when setting up the integration in [Step 5](https://www.assemblyai.com/docs/integrations/genesys-cloud#step-5-setting-up-audiohook-monitor).
...
This server is a method to get up and running quickly for development and testing purposes without the complexity of a production deployment. How you implement this in practice will vary widely depending on your based on traffic volume, scaling needs, reliability requirements, security concerns, etc.
</Note>
...
<Note>
`AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` can be found on your account's [IAM > Security Credentials page](https://us-east-1.console.aws.amazon.com/iam/home?region=us-east-1#/security_credentials). `API_KEY` is explained further in the [next step](https://www.assemblyai.com/docs/integrations/genesys-cloud#step-3-3), but it can be anything you want to verify that the requests are actually originating from Genesys.
</Note>
...
In the Genesys Cloud Admin section, navigate to the [Integrations](https://apps.usw2.pure.cloud/directory/#/admin/integrations/apps) page. Add a new integration via the plus sign in the top right corner. Search for **AudioHook Monitor** and install.

</Step>
<Step>

Navigate to the AudioHook Monitor's **Configuration** tab. Under the **Properties section**, make sure both channels are selected and the **Connection URI** is set to the ngrok url from the [previous step](https://www.assemblyai.com/docs/integrations/genesys-cloud#step-3-2). For the ngrok url, replace `https` with `wss`.
...
At the top of the Lambda function, you can edit the config to enable features for your transcripts. Since our call is two channels, we'll want to set `multichannel` to `true`. To see all available parameters, check out our [API reference](https://www.assemblyai.com/docs/api-reference/transcripts/submit).
```javascript
ASSEMBLYAI_CONFIG = {
  'multichannel': true,
  // 'language_code': 'en_us',
...
Now, navigate to the [S3](https://us-east-1.console.aws.amazon.com/s3) services page and click into the general purpose bucket where your Genesys recordings are stored. Browse to the **Properties** tab and then scroll down to **Event notifications**. Click **Create event notification**. Give the event a name and then in the **Prefix** section enter `calls/` (or whatever `S3_KEY_PREFIX` is set to), and in the **Suffix** section enter `.wav`. This will ensure the event is triggered once our `wav` file has been uploaded.
...
Scroll down to the **Destination** section, set the destination as **Lambda function** and then select the Lambda function we created in [Step 6](https://www.assemblyai.com/docs/integrations/genesys-cloud#step-2-4). Then click **Save changes**.
...
To test everything is working, call the phone number you linked to this flow in [Step 2](https://www.assemblyai.com/docs/integrations/genesys-cloud#step-2-setup-a-phone-and-routing-for-your-flow-optional). Referring to the example flow above, press the DTMF value on the key pad or say the Speech Recognition value. Once transferred, your WebSocket server should start to receive data and output to console:
...
Once you end the call, you should see the following logs:
...
<Step>
**Success!** You have successfully integrated AssemblyAI with Genesys Cloud via AudioHook Monitor. If you run into any issues or have further questions, please reach out to our Support team at `support@assemblyai.com`.
</Step>
</Steps>
...
### Multichannel
- As mentioned in [Step 6](https://www.assemblyai.com/docs/integrations/genesys-cloud#step-2-4), the `multichannel` parameter should be enabled as the files are stereo utilizing one channel for each participant. When possible, multiple channels are recommended by AssemblyAI for the most accurate transcription results.
