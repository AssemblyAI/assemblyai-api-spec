...
You can only enable one of the Summarization and [Auto Chapters](../03-audio-intelligence/auto-chapters.mdx) models in the same transcription.
</Warning>
...
If you want more control of the output format, see how to generate a [Custom summary using LeMUR](../04-lemur/summarize-audio.mdx).
</Tip>
...
| `conversational` | Best for any 2-person conversation, such as customer/agent or interview/interviewee calls. | `bullets`, `bullets_verbose`, `headline`, or `paragraph` | `punctuate` and `format_text` set to `true` |
