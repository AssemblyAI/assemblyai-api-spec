...
Make a GET request here `https://api.assemblyai.com/v2/transcript/:transcript_id/redacted-audio` and replace `transcript_id` with the ID of the transcript where `redact_pii_audio` was set to `true`. See the [API Reference Guide](https://www.assemblyai.com/docs/api-reference/transcripts/get-redacted-audio) for more information.
</Tip>

<Note title="Webhooks and PII Audio Redaction">

If using webhooks with PII audio redaction enabled, you'll receive two webhook calls: one when the full transcript completes and another when the redacted audio is ready. For more information about using webhooks, refer to our [webhook documentation](https://www.assemblyai.com/docs/deployment/webhooks).
...
# ...
redacted_audio_polling_endpoint = f"https://api.assemblyai.com/v2/transcript/{transcript_id}/redacted-audio"
...
while (true) {
  const redactedAudioPollingResponse = await axios.get(redactedAudioPollingEndpoint, {
    headers: headers
  })
...
# ...
$redacted_audio_polling_endpoint = "https://api.assemblyai.com/v2/transcript/" . $transcript_id . "/redacted-audio";
