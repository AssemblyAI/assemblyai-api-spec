...
<Tip title=" Redacted Audio Endpoint">
  ...
  `https://api.assemblyai.com/v2/transcript/:transcript_id/redacted-audio` and
  replace `transcript_id` with the ID of the transcript where `redact_pii_audio`
  was set to `true`. See the [API Reference
  Guide](/docs/api-reference/transcripts/get-redacted-audio) for more
  information.
</Tip>
