...
<Tabs groupId="language">
  ...
  To add speaker labels to each sentiment analysis result, using [Speaker Diarization](../02-speech-to-text/speaker-diarization.mdx), enable `speaker_labels` in the transcription config.
...
```
  ...
  To add speaker labels to each sentiment analysis result, using [Speaker Diarization](../02-speech-to-text/speaker-diarization.mdx), enable `SpeakerLabels` in the transcription parameters.
...
<CodeBlock>
  <JsonViewer
    ...
    collapsed={1}
    src={(({ sentiment_analysis_results }) => ({
...
| `sentiment_analysis_results[i].speaker` | string or null | The speaker of the i-th sentence if <a href="/speech-to-text/speaker-diarization">Speaker Diarization</a> is enabled, else null. |
...
The [Content Moderation model](../03-audio-intelligence/content-moderation.mdx) can be used to identify and filter out sensitive or offensive content from the transcript.
