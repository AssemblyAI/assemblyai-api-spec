...
<Warning>
  **LeMUR is deprecated and will be removed on March 31st, 2026.** Use the [LLM
  Gateway](/docs/llm-gateway/overview) instead, which provides access to 15+
  models with better performance. LLM Gateway is available via direct API calls
  — SDK support is not yet available. See the [Migration
  Guide](/docs/llm-gateway/migration-from-lemur) for details.
</Warning>
