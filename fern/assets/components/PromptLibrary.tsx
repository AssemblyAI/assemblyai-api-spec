...
export function PromptLibrary() {
  ...
  const [votedPrompts, setVotedPrompts] = React.useState<Set<string>>(
    new Set()
  );
  ...
  const handleSubmit = async () => {
    ...
    if (isSpam(newPrompt)) {
      setSubmitStatus({
        type: "error",
        message:
          "Your prompt appears to be spam. Please submit a valid transcription prompt.",
      });
      return;
    }
    ...
    try {
      ...
      setSubmitStatus({
        type: "success",
        message:
          "Thank you! Your prompt has been submitted for review. It will appear in the library once approved.",
      });
      setNewPrompt("");
    } catch (error) {
      const errorMessage =
  ...
  return (
    <div style={containerStyle}>
      <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
        ...
        {/* Search Results - Same card UI as top prompts, limited to 6 */}
        {searchQuery && (
          <div>
            <div style={sectionTitleStyle}>
              {Math.min(filteredPrompts.length, 6)} result
              {Math.min(filteredPrompts.length, 6) !== 1 ? "s" : ""} for "
              {searchQuery}"
              {filteredPrompts.length > 6
                ? ` (showing top 6 of ${filteredPrompts.length})`
                : ""}
            </div>
            {filteredPrompts.length === 0 ? (
              ...
              >
                No prompts found matching your search. Try different keywords or
                submit your own prompt below!
              </div>
            ) : (
              <div style={{ display: "flex", gap: "16px", flexWrap: "wrap" }}>
